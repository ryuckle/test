...
-(void)tabTaped:(UITapGestureRecognizer *)recognizer{
    ...
    Menu *tab = (Menu *)recognizer.view ;
    
    [self activateTabMenu:tab];
}

-(void)activateTabMenuWithIndex:(NSInteger)tabIndex
{
    for (Menu *tab in _tabs){
        if([self tagWithIndex:tabIndex] == tab.tag){
            [self activateTabMenu:tab];
        }
    }
}

-(void)activateTabMenu:(Menu*)tab
{
